function [usedLEAP,x,y,nx,ny,tx,ty,confb,confn,conftt,cx1,cy1,t,mL,sess,mouse,dT,dnT,nTr,t2f,t2s,f2s,f2f,sessT,mouseT,sT,t2s1,mTlen,mTlen2,C,sP,Tr,sessS,mouseS,initD,orient,initO,bV,nV,ndphi,bdphi,slope,Dline,theta,edge,cx0,cy0, Wind, fx0, fy0] = Wrap_WindExperiment()
...
if readXLS
    [num,text] = xlsread('Wind Experiment.xlsx','A1:D34');
    ...
    for iD=1:nD
        ...
        if sum(tempL)~=sum(tempB)
            ...
            elseif S > L % vid missing, must add at least 1 space to sListL
                if strcmp(dateStr,'2018-09-17') && sessNum(3)~=1 && mI==1
                    sListL{1,mI} = [nan,sListL{1,mI}(Lsess(1):end)];
                    sListL{2,mI} = [nan,sListL{2,mI}(Lsess(1):end)];
                elseif strcmp(dateStr,'2018-10-22') && sessNum(4)~=4 && mI==2
                ...
                elseif strcmp(dateStr,'2018-11-14') && sessNum(1)~=1 && mI==2
                ...
                elseif strcmp(dateStr,'2018-11-16') && sessNum(4)~=4 && mI==2
                    sListL{1,mI} = [sListL{1,mI}(1:Lsess(3)),nan,sListL{1,mI}(Lsess(4):end)];
                    sListL{2,mI} = [sListL{2,mI}(1:Lsess(3)),nan,sListL{2,mI}(Lsess(4):end)];
                elseif strcmp(dateStr,'2018-11-30') && sessNum(4)~=4 && mI==2
                    sListL{1,mI} = [sListL{1,mI}(1:Lsess(3)),nan,sListL{1,mI}(Lsess(4):end)];
                    sListL{2,mI} = [sListL{2,mI}(1:Lsess(3)),nan,sListL{2,mI}(Lsess(4):end)];
                elseif strcmp(dateStr,'2018-10-12') && sessNum(1)~=1 && mI==4
                    sListL{1,mI} = [sListL{1,mI}(1:Lsess(1)-1),nan,sListL{1,mI}(Lsess(1):end)];
                    sListL{2,mI} = [sListL{2,mI}(1:Lsess(1)-1),nan,sListL{2,mI}(Lsess(1):end)];
                else
                    keyboard;
        ...
        for iT=1:nT
            ...
            % Wind experiment concentration all 0.01% 2019-01-23 AndyP
            C(mI,iC) = 1; % unscented->0,0.01%->1,0.1%->2,1%->3,2%->4,no spot->-1
            %             switch siD{iT}
            %                 case 'A'
            %                     s2n = 1;
            %                 case 'B'
            ...
            pos1 = strfind(siD{iT},'1');
            pos2 = strfind(siD{iT},'2');
            pos3 = strfind(siD{iT},'3');
            pos4 = strfind(siD{iT},'4');
            nanpos = strfind(siD{iT},'nan');
            if ~isempty(pos1)
                s2n = 1;
            elseif ~isempty(pos2)
                s2n = 2;
            ...
            elseif ~isempty(pos4)
                s2n = 4;
            elseif ~isempty(nanpos)
                s2n = nan;
            end
            assert(~all(isempty(cat(1,pos1,pos2,pos3,pos4,nanpos))),'spot position xls error');
            assert(sum(~isempty(cat(1,pos1,pos2,pos3,pos4,nanpos)))==1,'spot position xls error');
            sP(mI,iC) = s2n; % sP = []; % spot position (A-N -> 1-14)
            
            
            w1 = strfind(siD{iT},'ON');
            w2 = strfind(siD{iT},'OFF');
            nanw = strfind(siD{iT},'nan');
            if ~isempty(w1)
                w0 = 1;
            elseif ~isempty(w2)
                w0 = 0;
            ...
            assert(~all(isempty(cat(1,w1,w2,nanw))),'wind xls error');
            assert(sum(~isempty(cat(1,w1,w2,nanw)))==1,'wind xls error');
    ...
    for iS=1:nS
        if ~isnan(Wind(iM,iS))
            ...
            if ~isnan(sDprimary{iC})
                primaryflag = 1;
                load(sDprimary{iC},'bx','by','nx','ny','ttx','tty','confb','confn','conftt');
                x0 = bx;
                y0 = by;
                nx0 = nx;
                ny0 = ny;
                ...
                primaryflag = 0;
                load(sDsecondary{iC},'x0','y0','nx0','ny0','tx0','ty0','fanx','fany');
            end
            % load the rest
            load(sDsecondary{iC},'fanx','fany','cx','cy','time0','frame0','mouseLength0','trialNum','trialStartT','foundSpotT','maxSessT','radiusAroundSpot','pixpercm');
            ...
            mL = cat(1,mL,sqrt((nx0-ttx0).^2+(ny0-tty0).^2)./pixpercm);
            cx1 = cat(1,cx1,repmat(cx,[nP,1]));
            cy1 = cat(1,cy1,repmat(cy,[nP,1]));
            dT = cat(1,dT,sqrt((x0-cx).^2+(y0-cy).^2)./pixpercm);
            dnT0 = sqrt((nx0-cx).^2+(ny0-cy).^2)./pixpercm;
            dnT = cat(1,dnT,dnT0);
