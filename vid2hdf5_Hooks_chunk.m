function vid2hdf5_Hooks(videoPath,frame2start)
...
if frame2start==1
    savePath = strcat(strcat(saveStr{1},'_chunk1'),'.h5');
else
    savePath = strcat(strcat(saveStr{1},'_chunk2'),'.h5');
end
...
for iT=1:nT
    ...
    background = cat(3,background,imadjust(I,[0.4,1]));
...
framesWritten = 0;
t0 = tic;
if vr.Duration > frame2start/vr.FrameRate
    vr.CurrentTime = frame2start/vr.FrameRate;
    
    while framesWritten < 7000 && ~done
        % Read next frame
