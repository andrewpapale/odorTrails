function [sumTimmobile,bin_sumTimmobile,immobile,V,nV] = getImmobileTimes2(x0,y0,nx0,ny0,arena_data,thresholdbody,thresholdnose,bins,timethreshold,timebins)
% 2017-10-13 AndyP
...
bT = linspace(1,length(immobile),ceil(length(immobile)/(30*timebins)));
bin_sumTimmobile = histcounts(find(immobile),bT);
bin_sumTimmobile = bin_sumTimmobile/30;
